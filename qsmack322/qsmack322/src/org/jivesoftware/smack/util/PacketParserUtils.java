...
public class PacketParserUtils {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (elementName.equals("error")) {
					...
					iqPacket = parseRegistration(parser);
						...
						&& namespace.equals("jabber:iq:time")) {
					iqPacket=parseTime(parser) ;
	...
	public static PacketExtension parsePacketExtension(String elementName,
		...
		// System.out.println("do parse--"+elementName+"vs"+namespace);
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				String value = parser.getText();
				extension.setValue(elementName, value);
	...
	private static Time parseTime(XmlPullParser parser)
			throws XmlPullParserException, IOException {
		Time time = new Time();
		boolean done = false;
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (elementName.equals("utc")) {
					time.setUtc(elementValue) ;
				}else if(elementName.equals("tz")){
					time.setTz(elementValue);
				}else if(elementName.equals("display")){
					time.setDisplay(elementValue) ;
				}
			} else if (eventType == XmlPullParser.END_TAG) {
