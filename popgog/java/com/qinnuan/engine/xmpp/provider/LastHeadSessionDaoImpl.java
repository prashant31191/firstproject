 ...
 */
public class LastHeadSessionDaoImpl extends AbstractSessionDao {
    public LastHeadSessionDaoImpl(Context context) {
        super(context);
    }
    ...
    public List<MessageListBean> getSession() {
        ...
        if(cursor.getCount()!=0){
            ...
            while (cursor.moveToNext()){
                ...
                bean.setType(SessionType.LASTHEAD);
                listBeans.add(bean) ;
            }
            cursor.close();
            return listBeans;
    ...
    private void updateNumberSession(MessageListBean bean) {
        contentResolver.update(uri, getContentValues(bean), ChatSession.Session.USER_ID + "=? AND "
                + ChatSession.Session.MESSAGE_TYPE + " = ?", new String[]{Const.user.getUserid(), SessionType.LASTHEAD.toString()});
    }
    ...
    private Cursor getNumberSession() {
        return contentResolver.query(uri, project, ChatSession.Session.USER_ID + "=? AND "
                ...
                new String[]{Const.user.getUserid(), SessionType.LASTHEAD.toString()}, ChatSession.Session.DEFAULT_SORT_ORDER);
    }
}
